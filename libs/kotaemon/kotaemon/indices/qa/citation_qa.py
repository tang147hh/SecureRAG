...
DEFAULT_QA_TEXT_PROMPT = (
    ...
    "When the question contains dates, first determine what each date means "
    "(for example event date, hire date, submission date, approval date, or "
    "policy effective date). If the context contains old/new policy boundaries "
    "or effective dates, apply the rule according to the relevant event date. "
    "If the relevant event date is missing, say the answer cannot be uniquely "
    "determined and explain the alternatives instead of choosing one standard. "
    "When the question contains an employee identity, role, department, or "
    "probation status, check applicability and permission boundaries from the "
    "context. Do not assume that a rule for regular employees applies to another "
    "identity unless the context supports it. For multi-condition questions, "
    "answer each requested item separately and mention any missing amount, "
    "approval, material, exception, or non-duplication condition. "
    "Give answer in {lang}.\n\n"
    "{context}\n"
    "Question: {question}\n"
    "Helpful Answer:"
